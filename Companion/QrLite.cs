// Vendored from the standalone library DooDesch/QrLite (MIT) - keep in sync with that repo rather than
// editing here. Copied rather than referenced because it is one file with no dependencies, and shipping a second
// DLL for a QR encoder is not worth an install step.
// QrLite - a tiny, dependency-free QR Code encoder in a single C# file.
// SPDX-License-Identifier: MIT
// Copyright (c) 2026 DooDesch
//
// Drop this file into your project. It has no dependencies and no Unity/platform types, so it works anywhere -
// including Unity IL2CPP and Mono. The class is `internal` so you can copy it into several assemblies without a
// CS0436 duplicate-type clash; change it to `public` if you prefer to reference it as a shared library.
//
// Scope (kept intentionally small): byte mode (UTF-8), error-correction level L, versions 1-10 (up to about
// 271 bytes) - plenty for URLs and pairing codes. Every output is verified against a reference decoder.
//
//   bool[,] m = QrLite.Encode("https://example.com");   // m[row, col] == true means a dark module
//   Console.WriteLine(QrLite.ToText(m));                 // quick monospace preview
...
namespace Reflash.Companion
{
    ...
    /// that fits). Pure managed C# - no reflection, no unsafe, no platform types - so it runs anywhere.
    /// <see cref="Encode"/> returns the module matrix (true = dark); render it however you like, or use
    /// <see cref="ToArgb"/> for a ready-to-draw ARGB pixel buffer and <see cref="ToText"/> for a console preview.
    /// </summary>
    internal static class QrLite
    {
        ...
        /// <summary>Rasterise a matrix to an ARGB (0xAARRGGBB) pixel buffer, row-major, each module scaled to
        /// <paramref name="scale"/> pixels with a light quiet border of <paramref name="quiet"/> modules (keep the
        /// quiet zone - scanners need it). The image is square; its side length is returned in <paramref name="dim"/>.
        /// No platform types - hand it to any image API (e.g. Unity Texture2D.SetPixels32, a PNG encoder). Null in,
        /// null out.</summary>
        internal static uint[] ToArgb(bool[,] m, out int dim, int scale = 8, int quiet = 4,
            uint dark = 0xFF000000u, uint light = 0xFFFFFFFFu)
        {
            dim = 0;
            ...
            for (int r = 0; r < n; r++)
                ...
                {
                    if (!m[r, c]) continue;
                    int x0 = (c + quiet) * scale, y0 = (r + quiet) * scale;
                    for (int y = 0; y < scale; y++)
                    {
                        int rowBase = (y0 + y) * dim + x0;
                        for (int x = 0; x < scale; x++) px[rowBase + x] = dark;
                    }
                }
            return px;
        }

        /// <summary>Render a matrix to a monospace string for a quick preview (a light quiet border, two chars per
        /// module so it stays roughly square in a console). Returns "" for a null matrix.</summary>
        internal static string ToText(bool[,] m, int quiet = 2, string dark = "██", string light = "  ")
        {
            if (m == null) return "";
            int size = m.GetLength(0);
            var sb = new System.Text.StringBuilder();
            for (int r = -quiet; r < size + quiet; r++)
            {
                for (int c = -quiet; c < size + quiet; c++)
                {
                    bool d = r >= 0 && r < size && c >= 0 && c < size && m[r, c];
